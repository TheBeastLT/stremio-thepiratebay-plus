...
const defaultProxies = process.env.PROXIES ? process.env.PROXIES.split(',') : ['https://pirateproxy.sh'];
...
function singleRequest(keyword, url, config = {}) {
  ...
  const category = config.cat || categories.Video;
  ...
  return new Promise((resolve, reject) => {
    ...
    $('table[id=\'searchResult\'] tr').each(function() {
      const torrent = {
        ...
        infoHash: decode($(this).find('a[title=\'Download this torrent using magnet\']').attr('href')).infoHash
      };
