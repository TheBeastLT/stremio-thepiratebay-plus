...
const METADATA_TTL = process.env.METADATA_TTL || 7 * 24 * 60 * 60 * 1000; // 7 days
const TORRENT_TTL = process.env.TORRENT_TTL || 72 * 60 * 60 * 1000; // 72 hours
const TORRENT_FILES_TTL = process.env.TORRENT_FILES_TTL || 30 * 24 * 60 * 60 * 1000; // 30 days
const STREAM_TTL = process.env.STREAM_TTL || 48 * 60 * 60 * 1000; // 48 hours
const STREAM_EMPTY_TTL = process.env.STREAM_EMPTY_TTL || 30 * 60 * 1000; // 30 minutes
...
function initiateCache() {
  const innerStore = MONGO_URI && new KeyvMongo(MONGO_URI, { collection: 'tpb-addon-collection', socketTimeoutMS: 30000 })
