...
function canContainEpisode(torrent, seriesInfo, seasonInfoOnly = false) {
  ...
  const escapedCommunityTitle = seriesInfo.communityTitle &&
    seriesInfo.communityTitle.replace('(', '\\(').replace(')', '\\)');
  const titleRegex = new RegExp(`(?:^|\\()(?:${seriesInfo.title}|${escapedCommunityTitle})`, 'i');
